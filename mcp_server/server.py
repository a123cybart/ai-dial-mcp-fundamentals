...
async def get_user_by_id(user_id: int) -> str:
    ...
    return user_info if user_info else "User not found."
...
@mcp.tool()
async def search_users(user_search_request: UserSearchRequest) -> str:
    """Searches for users based on provided criteria."""

    users = await user_client.search_users(
        **user_search_request.model_dump(exclude_none=True)
    )
    return users if users else "No users found matching the criteria."
...
async def update_user(user_id: int, user_update_model: UserUpdate) -> str:
    """Updates an existing user's information using their unique ID."""
...
@mcp.prompt(
    name="user_profile_creation_guide",
)
async def user_profile_creation_guide() -> str:
    """Guides the creation of realistic user profiles for the system."""
    return """
